...
void CHudSpectator::DrawOverviewEntities()
{
	...
	{
		if (0 == m_OverviewEntities[i].V_HSPRITE)
			continue;

		V_HSPRITEModel = (struct model_s*)gEngfuncs.GetSpritePointer(m_OverviewEntities[i].V_HSPRITE);
		ent = m_OverviewEntities[i].entity;

		gEngfuncs.pTriAPI->SpriteTexture(V_HSPRITEModel, 0);
		gEngfuncs.pTriAPI->RenderMode(kRenderTransTexture);
		...
		V_HSPRITEModel = (struct model_s*)gEngfuncs.GetSpritePointer(m_hsprBeam);
		gEngfuncs.pTriAPI->SpriteTexture(V_HSPRITEModel, 0);
	...
	V_HSPRITEModel = (struct model_s*)gEngfuncs.GetSpritePointer(m_hsprCamera);
	gEngfuncs.pTriAPI->RenderMode(kRenderTransAdd);
	gEngfuncs.pTriAPI->SpriteTexture(V_HSPRITEModel, 0);
...
bool CHudSpectator::AddOverviewEntity(int type, struct cl_entity_s* ent, const char* modelname)
{
	...
	if (type == ET_PLAYER)
	{
		if (ent->curstate.solid != SOLID_NOT)
		{
			...
			case 1:
				V_HSPRITE = m_hsprPlayerBlue;
				break;
			case 2:
				V_HSPRITE = m_hsprPlayerRed;
				break;
			default:
				V_HSPRITE = m_hsprPlayer;
				break;
			}
		}
		else
	...
	return AddOverviewEntityToList(V_HSPRITE, ent, gEngfuncs.GetClientTime() + duration);
}
...
bool CHudSpectator::AddOverviewEntityToList(V_HSPRITE sprite, cl_entity_t* ent, double killTime)
{
	for (int i = 0; i < MAX_OVERVIEW_ENTITIES; i++)
	{
		...
		if (m_OverviewEntities[i].entity == NULL)
		{
			...
			m_OverviewEntities[i].V_HSPRITE = sprite;
