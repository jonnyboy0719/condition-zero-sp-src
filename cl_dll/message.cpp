...
void CHudMessage::MessageDrawScan(client_textmessage_t* pMessage, float time)
{
	...
	if ( pMessage->pMessage[0] == '#' )
		pText = Localize::GetLocalizedText( pMessage->pMessage );
	else
	...
	for (i = 0; i < maxHUDMessages; i++)
	{
		if (m_pMessages[i])
		{
			...
			case 2:
				{
					if ( pMessage->pMessage[0] == '#' )
						endTime = m_startTime[i] + (pMessage->fadein * strlen(Localize::GetLocalizedText( pMessage->pMessage ))) + pMessage->fadeout + pMessage->holdtime;
					else
...
void CHudMessage::MessageSetChannel(int channel, int flags, client_textmessage_t* newMessage)
{
	m_parms.time = gHUD.m_flTime;
	...
	if (channel < 0)
		channel = 0;
	else if (channel > maxHUDMessages)
		channel = maxHUDMessages - 1;

	if (!m_pMessages[channel])
	{
		m_pMessages[channel] = newMessage;
		m_startTime[channel] = gHUD.m_flTime;
	}
}
