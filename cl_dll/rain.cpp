...
/*
	if (gHUD.ProcessWeather->value == 2)
		rain_nextspawntime = gEngfuncs.GetClientTime() + 5;

	if (gHUD.ProcessWeather->value == 3)
		rain_nextspawntime = gEngfuncs.GetClientTime() + 10;
*/
	// first frame
	if (rain_oldtime == 0)
	{
	...
	while (curDrip != NULL) // go through list
	{
		...
		{
			if (Rain.weatherMode == WEATHER_RAIN)
	...
	int iQuality = int(gHUD.ProcessWeather->value);
	switch ( iQuality )
	{
	...
	while (rain_nextspawntime < rain_curtime)
	{
		...
		if (dripcounter < max_drawamount) // check for overflow
		{
			...
			{
				gEngfuncs.Con_Printf("Weather error: can't create drip in water\n");
				continue;
			}
			...
			if (!newClDrip)
			{
				...
				gEngfuncs.Con_Printf( "Weather error: failed to allocate object!\n");
				return;
			}
...
#if 0
	if (gHUD.RainInfo->value > 0) // print debug info
	{
	...
	else
	{
		...
		*ch = '\0';
		...
		if ( ch )
		{
			...
			memmove( mapname, ch+1, strlen(ch)+1 );
		}
	}
*/
	if (strlen(mapname) == 0)
	{
		gEngfuncs.Con_Printf( "Weather error: unable to read map name\n");
		return;
	}
	...
	/*
	strcpy(szFilename1, "scripts/fx/");
	strcat ( szFilename1, mapname );
	strcat( szFilename1, ".pcs" );//txt?
	strcat( szFilename2, szFilename1 );
	strcat( szFilename2, "\n" );
*/

//	pfile = (char *)gEngfuncs.COM_LoadFile( szFilename2, 5, NULL);
	pfile = (char *)gEngfuncs.COM_LoadFile( mapname, 5, NULL);

	if (!pfile)
	{
		if (gHUD.RainInfo->value > 0)
			gEngfuncs.Con_Printf("Weather: couldn't open rain settings file %s\n", mapname);
		return;
	}
	...
	while (true)
	{
		...
		else if (!stricmp(token, "height")) // globalHeight
		{
			...
			gEngfuncs.Con_Printf("Weather error: unknown token %s in file %s\n", token, mapname);
	}
