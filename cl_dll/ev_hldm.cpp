...
void EV_HLDM_FireBullets(int idx, float* forward, float* right, float* up, int cShots, float* vecSrc, float* vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int* tracerCount, float flSpreadX, float flSpreadY, bool randomspread)
{
	...
	for (iShot = 1; iShot <= cShots; iShot++)
	{
		...
		float x, y, z;
		//We randomize for the Shotgun. Or if we have random spread enabled
		if (iBulletType == BULLET_PLAYER_BUCKSHOT || randomspread)
		{
			do
			{
...
void EV_FireGlock(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		if ( bDoEventSeq )
		{
			int anim = bBurstShot ? GLOCK_SHOOT : GLOCK_SHOOT_SINGLE;
			if (empty) anim = GLOCK_SHOOT_EMPTY;
			gEngfuncs.pEventAPI->EV_WeaponAnimation(anim, 0);
	...
	if ( bDoEventSeq )
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, bBurstShot ? "weapons/glock18-1.wav" : "weapons/glock18-2.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong(0, 3));

	EV_GetGunPosition(args, vecSrc, origin);

	VectorCopy(forward, vecAiming);

	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_9MM, 0, &tracerCount[idx - 1], args->fparam1, args->fparam2, bBurstShot);
}
//======================
//	   GLOCK END
//======================
...
void EV_FireUSP(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(empty ? USP_SHOOT_EMPTY : USP_SHOOT, 0);
...
void EV_FireUMP45(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		{
			case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation(UMP45_FIRE1, 0); break;
			case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation(UMP45_FIRE2, 0); break;
			case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation(UMP45_FIRE3, 0); break;
		}
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/ump45-1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong(0, 3));
...
void EV_FireMAC10(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		switch (gEngfuncs.pfnRandomLong(0, 2))
		{
			case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation(MAC10_FIRE1, 0); break;
			case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation(MAC10_FIRE2, 0); break;
			case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation(MAC10_FIRE3, 0); break;
		}
		V_PunchAxis(0, gEngfuncs.pfnRandomFloat(-2, 2));
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/mac10-1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong(0, 3));
...
void EV_FireAK47(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		switch (gEngfuncs.pfnRandomLong(0, 2))
		{
			case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation(AK47_FIRE1, 0); break;
			case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation(AK47_FIRE2, 0); break;
			case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation(AK47_FIRE3, 0); break;
		}
		V_PunchAxis(0, gEngfuncs.pfnRandomFloat(-2, 2));
	}
...
void EV_FireM4A1(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(M4A1_FIRE, 0);
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/m4a1_-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/m4a1_-2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	}
...
void EV_Knife(event_args_t* args)
{
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/knife_slash1.wav", 1, ATTN_NORM, 0, PITCH_NORM);
		break;
	case 1:
...
void EV_FireRpg(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(RPG_FIRE, 0);
