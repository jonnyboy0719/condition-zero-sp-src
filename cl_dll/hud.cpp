...
cvar_t* cl_lang = nullptr;
cvar_t* cl_lang_debug = nullptr;
cvar_t* cl_fog = nullptr;
...
{
	return static_cast<int>(gHUD.MsgFunc_RainData(pszName, iSize, pbuf));
}
...
int __MsgFunc_ScritSeq(const char* pszName, int iSize, void* pbuf)
{
	return static_cast<int>(gHUD.MsgFunc_ScritSeq(pszName, iSize, pbuf));
}

// Objectives
void __CmdFunc_OpenObjectives()
{
	gHUD.m_Objectives.Show();
}
void __CmdFunc_CloseObjectives()
{
	gHUD.m_Objectives.Hide();
}
...
int __MsgFunc_DCross(const char* pszName, int iSize, void* pbuf)
{
	return static_cast<int>(gHUD.MsgFunc_DynCrosshair(pszName, iSize, pbuf));
}
...
void __CmdFunc_ReloadLanguageFile()
{
	Localize::Reload();
}
...
void CHud::Init()
{
	...
	HOOK_COMMAND("+mission", OpenObjectives);
	HOOK_COMMAND("-mission", CloseObjectives);
	...
	cl_fog = CVAR_CREATE("cl_fog", "1", FCVAR_ARCHIVE);
	cl_lang = CVAR_CREATE("cl_lang", "english", FCVAR_ARCHIVE);
	cl_lang_debug = CVAR_CREATE("cl_lang_debug", "0", FCVAR_CLIENTDLL);
	HOOK_COMMAND("cl_lang_reload", ReloadLanguageFile);

	// From AM Rebirth
	RainInfo = gEngfuncs.pfnRegisterVariable( "cl_raininfo", "0", 0 );
	ProcessWeather = CVAR_CREATE("cl_weather", "3", FCVAR_ARCHIVE);
...
// returns an index into the gHUD.m_rgV_HSPRITEs[] array
...
void CHud::VidInit()
{
	...
	if (!m_pSpriteList)
	{
		...
		if (m_pSpriteList)
		{
			...
			m_rgV_HSPRITEs = new V_HSPRITE[m_iSpriteCount];
			...
			for (j = 0; j < m_iSpriteCountAllRes; j++)
			{
				if (p->iRes == m_iRes)
				{
					...
					m_rgV_HSPRITEs[index] = SPR_Load(sz);
