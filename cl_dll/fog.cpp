...
{
	// Check if we should draw fog or not
	bool bFogDisabled = (cl_fog->value <= 0) ? true : false;

	// If start distance and end distance isn't properly set
	if ( !(m_fogParams.enddist > 0) && !(m_fogParams.startdist > 0) )
		bFogDisabled = true;
	...
	glFogf(GL_FOG_DENSITY, m_fogParams.fogdense);
...
void CFog::BlendFog( void )
{
	...
	m_fogParams.fogdense = (m_fogBlend1.fogdense * (1.0 - interp)) + (m_fogBlend2.fogdense * interp);
}
...
int CFog::MsgFunc_Fog( const char *pszName, int iSize, void *pBuf )
{
	...
	if(blendtime > 0 && m_fogParams.enddist > 0 && m_fogParams.startdist > 0)
	{
		...
		m_fogBlend2.fogdense = fogdensity;
	...
	else
	{
		...
		m_fogParams.fogdense = fogdensity;
