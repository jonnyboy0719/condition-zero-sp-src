...
private:
	V_HSPRITE m_V_HSPRITE1;
	V_HSPRITE m_V_HSPRITE2;
	Rect* m_prc1;
	Rect* m_prc2;
	int m_iBat;
	int m_iBatMax;
...
public:
	...
	void MessageSetChannel(int channel, int flags, client_textmessage_t* newMessage);
...
private:
	...
	// freed in ~CHud()
	V_HSPRITE* m_rgV_HSPRITEs; /*[HUD_SPRITE_COUNT]*/ // the sprites loaded from hud.txt
...
public:
	...
	{
		return (index < 0) ? 0 : m_rgV_HSPRITEs[index];
	}
	...
	int GetSpriteIndex(const char* SpriteName); // gets a sprite index, for use in the m_rgV_HSPRITEs[] array
