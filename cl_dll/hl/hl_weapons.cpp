...
CFlashGrenade g_HandGrenF;
CSmokeGrenade g_HandGrenS;
CNervGasGrenade g_HandGrenN;
#ifdef HL_WEAPONS
CSatchel g_Satchel;
CTripmine g_Tripmine;
CSqueak g_Snark;
#endif
CRadioBombController g_RadioController;
...
void HUD_InitClientWeapons()
{
	...
	HUD_PrepEntity(&g_HandGrenF, &player);
	HUD_PrepEntity(&g_HandGrenS, &player);
	HUD_PrepEntity(&g_HandGrenN, &player);
#ifdef HL_WEAPONS
	HUD_PrepEntity(&g_Satchel, &player);
	HUD_PrepEntity(&g_Tripmine, &player);
	HUD_PrepEntity(&g_Snark, &player);
#endif
	HUD_PrepEntity(&g_RadioController, &player);
}
...
void HUD_WeaponsPostThink(local_state_s* from, local_state_s* to, usercmd_t* cmd, double time, unsigned int random_seed)
{
	...
	case WEAPON_SNARK:
		...
		break;
#endif
	case WEAPON_RADIOCONTROLLER:
		pWeapon = &g_RadioController;
		break;
	}
