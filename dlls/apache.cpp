...
TYPEDESCRIPTION CApache::m_SaveData[] =
	{
		...
		DEFINE_FIELD( CApache, snd_idle, FIELD_STRING ),
		DEFINE_FIELD( CApache, snd_death, FIELD_STRING ),
		DEFINE_FIELD( CApache, snd_fire, FIELD_STRING ),
...
void CApache::Precache()
{
	...
	PRECACHE_MODEL("models/blackhawk_gunship.mdl");

	// Custom Audio
	if (!FStringNull(snd_idle)) PRECACHE_SOUND(STRING(snd_idle));
	if (!FStringNull(snd_fire)) PRECACHE_SOUND(STRING(snd_fire));
	if (!FStringNull(snd_death)) PRECACHE_SOUND(STRING(snd_death));

	// HIND
	PRECACHE_SOUND("vehicles/hind_loop.wav");
	PRECACHE_SOUND("vehicles/hind_fire.wav");
	PRECACHE_SOUND("vehicles/hind_death.wav");

	// HARRIER
	PRECACHE_SOUND("vehicles/harrier_loop.wav");
	PRECACHE_SOUND("vehicles/harrier_fire.wav");
	PRECACHE_SOUND("vehicles/harrier_death.wav");
...
void CApache::Killed(entvars_t* pevAttacker, int iGib)
{
	...
	STOP_SOUND(ENT(pev), CHAN_STATIC, STRING(snd_idle));
	EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(snd_death), 1.0, 0.3, 0, 110);
...
void CApache::DyingThink()
{
	...
	else
	{
		...
		*/
		...
		STOP_SOUND(ENT(pev), CHAN_STATIC, STRING(snd_death));
...
void CApache::Flight()
{
	...
	if (m_iSoundState == 0)
	{
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(snd_idle), 1.0, 0.3, 0, 110);
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(snd_idle), 1.0, 0.3, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
		}
...
bool CApache::FireGun()
{
	...
	if (DotProduct(vecGun, vecTarget) > 0.98)
	{
		...
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, STRING(snd_fire), 1, 0.3);
#else
		static float flNext;
		TraceResult tr;
...
bool CApache::KeyValue(KeyValueData* pkvd)
{
	...
	{
		m_ExplodeOnDeath = atoi(pkvd->szValue) >= 1 ? true : false;
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "idlesound"))
	{
		snd_idle = ALLOC_STRING(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "deathsound"))
	{
		snd_death = ALLOC_STRING(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "firesound"))
	{
		snd_fire = ALLOC_STRING(pkvd->szValue);
		return true;
	}
