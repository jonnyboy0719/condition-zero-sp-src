...
Activity CBaseMonster::GetDeathActivity()
{
	...
	{
		ALERT(at_console, "Entity died and wants to play sequence: %i\n", m_iDeathSequencePlay);
		pev->frame = 0;
		pev->sequence = m_iDeathSequencePlay;
		pev->angles = Vector(0, m_iDeathAngle, 0);
		ResetSequenceInfo();
		SetYawSpeed();
		return ACT_DIECUSTOMANIM;
	}
...
void CBaseMonster::RadiusGas(Vector vecSrc, float size, entvars_t* pevInflictor, entvars_t* pevAttacker, bool bNervGas)
{
	CBaseEntity* pEntity = NULL;
	TraceResult tr;
	Vector vecSpot;
	const int bitsDamageType = DMG_NERVEGAS;
	const bool bInWater = (UTIL_PointContents(vecSrc) == CONTENTS_WATER);

	vecSrc.z += 1; // in case grenade is lying on the ground

	// iterate on all entities in the vicinity.
	while ((pEntity = UTIL_FindEntityInSphere(pEntity, vecSrc, size)) != NULL)
	{
		if (pEntity->pev->takedamage != DAMAGE_NO)
		{
			// blast's don't tavel into or out of water
			...
			// If this entity has a gasmask, no nervgas dmg.
			if ( pNPC && pNPC->m_gasmask )
				continue;
			...
			{
				if ( bNervGas )
					pEntity->TakeDamage(pevInflictor, pevAttacker, 5, bitsDamageType);
				if ( pNPC )
					pNPC->OnNPCCoughing();
			}
		}
		...
		if (tr.flFraction != 1.0)
		{
			...
			if (0 != iDamage)
			{
				...
				case BULLET_PLAYER_338MAGNUM:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgAWP, vecDir, &tr, DMG_BULLET);
					break;
