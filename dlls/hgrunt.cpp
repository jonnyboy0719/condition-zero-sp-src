...
LINK_ENTITY_TO_CLASS(monster_pmc, CHGrunt);
...
const char* CHGrunt::pGruntSentences[] =
{
	...
	"HG_ALERT",   // sees new enemy
	"HG_COVER",	  // running to cover
	"HG_THROW",	  // about to throw grenade
	"HG_CHARGE",  // running out to get the enemy
	"HG_TAUNT",	  // say rude things
	"T_FLASHED_AM", // flashbanged
	"T_SMOKED_AM",  // smokegrenade coughing
};
...
void CHGrunt::GibMonster()
{
	...
	{ // throw a gun if the grunt has one
		...
		{
			pGun = DropItem("weapon_xm1014", vecGunPos, vecGunAngles);
		}
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_DROP_GUN:
	{
		if (GetBodygroup(GUN_GROUP) != GUN_NONE)
		{
			...
			if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))
			{
				DropItem("weapon_xm1014", vecGunPos, vecGunAngles);
			}
			else if (FBitSet(pev->weapons, HGRUNT_WEPM4))
			{
	...
	case HGRUNT_AE_GREN_TOSS:
	{
		...
		CGrenade::ShootTimed(pev, GetGunPosition(), m_vecTossVelocity, 3.5, handgrenade_type_e::GRENADE_TYPE_EXPLOSIVE);
	...
	{
		UTIL_MakeVectors(pev->angles);
		CGrenade::ShootTimed(pev, pev->origin + gpGlobals->v_forward * 17 - gpGlobals->v_right * 27 + gpGlobals->v_up * 6, g_vecZero, 3, handgrenade_type_e::GRENADE_TYPE_EXPLOSIVE);
	}
	break;
	...
	case HGRUNT_AE_BURST1:
	{
		...
		else if (FBitSet(pev->weapons, HGRUNT_WEPM4))
		{
			...
			{
				EMIT_SOUND(ENT(pev), CHAN_WEAPON, "hgrunt/gr_mgun4.wav", 1, ATTN_NORM);
			}
		}
		else
		{
...
void CHGrunt::Spawn()
{
	...
	{
		int headgroup = HEAD_SHOTGUN;
		auto randomlong = RANDOM_LONG(0, 99);
		if (randomlong < 80)
			headgroup = HEAD_M203;
		else if (randomlong < 35)
			headgroup = HEAD_COMMANDER;
		SetBodygroup(HEAD_GROUP, headgroup);
	}
...
void CHGrunt::Precache()
{
	...
	PRECACHE_SOUND("hgrunt/gr_mgun4.wav");
...
Schedule_t slGruntGrenadeFlashed[] =
{
	{
		tlGruntGrenadeFlashed,
		ARRAYSIZE(tlGruntGrenadeFlashed),
		0,
		0,
		"GruntGrenadeFlashed"
	},
...
Schedule_t slGruntGrenadeSmoked[] =
{
	{
		tlGruntGrenadeSmoked,
		ARRAYSIZE(tlGruntGrenadeSmoked),
		0,
		0,
		"GruntGrenadeSmoked"
	},
...
DEFINE_CUSTOM_SCHEDULES(CHGrunt){
	...
	slGruntRepelAttack,
	slGruntRepelLand,
	slGruntGrenadeFlashed,
	slGruntGrenadeSmoked
};
...
{
	// Override the activity, if we have a death animation set.
	if (NewActivity == ACT_DIECUSTOMANIM)
		return;
	...
	case ACT_RANGE_ATTACK1:
		...
		if (FBitSet(pev->weapons, HGRUNT_9MMAR) || FBitSet(pev->weapons, HGRUNT_WEPM4))
		{
			if (m_fStanding)
			{
...
Schedule_t* CHGrunt::GetScheduleOfType(int Type)
{
	...
	{
		CTalkMonster::g_talkWaitTime = 0;
		m_iSentence = HGRUNT_SENT_FLASHBANGED;
		SpeakSentence();
		return &slGruntGrenadeFlashed[0];
	}
	case SCHED_CZERO_SMOKEGRENADE_COUGHING:
	{
		CTalkMonster::g_talkWaitTime = 0;
		m_iSentence = HGRUNT_SENT_SMOKEGRENADE_COUGHING;
		SpeakSentence();
		return &slGruntGrenadeSmoked[0];
	}
...
class CHGruntM4A1 : public CHGrunt
{
public:
	void Spawn() override;
};
LINK_ENTITY_TO_CLASS(monster_pmc_m4a1, CHGruntM4A1);


void CHGruntM4A1::Spawn()
{
	pev->weapons = HGRUNT_WEPM4;
	CHGrunt::Spawn();
}
...
LINK_ENTITY_TO_CLASS(monster_pmc_dead, CDeadHGrunt);
