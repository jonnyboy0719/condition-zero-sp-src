...
TYPEDESCRIPTION CBaseMonster::m_SaveData[] =
	{
		...
		DEFINE_FIELD(CBaseMonster, m_iDeathAnim, FIELD_STRING),
		DEFINE_FIELD(CBaseMonster, m_iDeathAngle, FIELD_INTEGER),

		DEFINE_FIELD(CBaseMonster, m_HackedGunPos, FIELD_VECTOR),

		// Save our death sequence
		DEFINE_FIELD(CBaseMonster, m_iDeathSequencePlay, FIELD_INTEGER),
		...
		DEFINE_FIELD(CBaseMonster, m_gasmask, FIELD_BOOLEAN),
		DEFINE_FIELD(CBaseMonster, m_cantmove, FIELD_BOOLEAN),
};
...
bool CBaseMonster::KeyValue(KeyValueData* pkvd)
{
	...
	{
		m_iDeathAngle = atoi(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "deathanim") || FStrEq(pkvd->szKeyName, "deadanim"))
	{
		m_iDeathAnim = ALLOC_STRING(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "cantmove"))
	{
		m_cantmove = atoi(pkvd->szValue) != 0;
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "invulnerable"))
	{
...
void CBaseMonster::SetupDeathAnimation()
{
	// Apply this after we spawned the monster entity.
	if (!FStringNull(m_iDeathAnim))
	{
		m_iDeathSequencePlay = LookupSequence(STRING(m_iDeathAnim));
		//DumpGetSequences();
	}
}
