...
edict_t *CBasePlayer::GiveNamedItem(const char* szName)
{
...
edict_t *CBasePlayer::GiveNamedItem(const char* szName, int defaultAmmo)
{
	auto pent = GiveNamedItem_Common(pev, szName);
	...
	if ( Rain_endFade > 0 )
	{
		...
		{ // we're in fading process
			if (Rain_nextFadeUpdate <= gpGlobals->time)
			{
				...
				ALERT(at_aiconsole, "weather fading: curdrips: %i, idealdrips %i\n", Rain_dripsPerSecond, Rain_ideal_dripsPerSecond);
			}
		}
		else
		{ // finish fading process
			...
			ALERT(at_aiconsole, "weather fading finished at %i drips\n", Rain_dripsPerSecond);
		}		
	}

	// send rain message
	if ( Rain_needsUpdate > 0 )
	{
	//search for env_weather_settings entity
		edict_t *pFind; 
		pFind = FIND_ENTITY_BY_CLASSNAME( NULL, "env_weather_settings" );
		if (!FNullEnt( pFind ))
		{
		// rain allowed on this map
			...
			pFind = FIND_ENTITY_BY_CLASSNAME( NULL, "env_weather_modify" );
			while ( !FNullEnt( pFind ) )
			{
				if (FBitSet(pFind->v.spawnflags, 1))
				{
				...
				pFind = FIND_ENTITY_BY_CLASSNAME( pFind, "env_weather_modify" );
			}
			...
			if ( Rain_dripsPerSecond > 0 )
				ALERT(at_aiconsole, "Sending enabling weather message\n");
			else
				ALERT(at_aiconsole, "Sending disabling weather message\n");
		}
		else
		{ // no rain on this map
