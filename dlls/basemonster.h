...
#include "angelscript/TriggerScript.h"
#define DEFINE_BASE( _BASE ) typedef _BASE CBase;
...
public:
	...
	string_t m_iDeathAnim;	// Our death animation to play
	int m_iDeathSequencePlay = -1;
	int m_iDeathAngle;		// Our desired angle for our death animation
	...
	bool m_gasmask = false;
	bool m_cantmove = false;
	...
	virtual void SetupDeathAnimation();
...
public:
	...
	void RadiusFlash(Vector vecSrc, entvars_t* pevInflictor, entvars_t* pevAttacker);
	void RadiusGas(Vector vecSrc, float size, entvars_t* pevInflictor, entvars_t* pevAttacker, bool bNervGas );
	bool IsMoving() override { return m_movementGoal != MOVEGOAL_NONE; }

	void RouteClear();
	void RouteNew();

	virtual void OnNPCCoughing() {}
	virtual void OnNPCFlashed() {}
