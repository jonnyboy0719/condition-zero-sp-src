...
{
	// studiomdl refuses to read new act tables, so let's override them here instead...
	//ALERT(at_console, "%s is playing activity: %s\n", STRING(pev->classname), activity_map[activity].name);
	if ( activity == ACT_CZERO_FLASHBANGED )
		return LookupSequence( "react_flashed" );
	else if ( activity == ACT_CZERO_SMOKECOUGH )
		return LookupSequence("react_smokeed");
...
void CBaseAnimating::DumpGetSequences()
{
	void* pmodel = GET_MODEL_PTR(ENT(pev));
	::DumpGetSequences(pmodel);
}
