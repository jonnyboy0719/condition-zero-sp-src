...
//
class CTriggerCDAudio : public CPointEntity
{
public:
	void Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value) override;
	void PlayTrack();
...
bool CTriggerCDAudio::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "trackname"))
	{
		m_trackname = ALLOC_STRING(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "loop"))
	{
		m_looptrack = atoi(pkvd->szValue) >= 1 ? true : false;
		return true;
	}
...
{
	// If we have a valid MP3 file, then play it!
	if (m_trackname != -1)
	{
		CBaseEntity* pClient = UTIL_GetLocalPlayer();
		if (pClient)
