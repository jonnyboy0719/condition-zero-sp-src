...
TYPEDESCRIPTION CEnvFog::m_SaveData[] =
	{
		...
		DEFINE_FIELD(CEnvFog, m_flDense, FIELD_FLOAT),
};
...
bool CEnvFog::KeyValue(KeyValueData* pkvd)
{
	...
	// Fix for CZero maps, other than that, never used.
	else if (FStrEq(pkvd->szKeyName, "density"))
	{
		m_flDense = atof(pkvd->szValue);
		return true;
	}
	else
		return CBaseEntity::KeyValue(pkvd);
...
LINK_ENTITY_TO_CLASS( env_weather_settings, CRainSettings );
...
LINK_ENTITY_TO_CLASS( env_weather_modify, CRainModify );
...
LINK_ENTITY_TO_CLASS( env_weather, CEnvRain );
...
{
	m_spriteTexture = PRECACHE_MODEL( (char *)STRING(m_iszSpriteName) );
	PRECACHE_MODEL("sprites/effects/exp_end.spr");
}
...
{
	if (!ShouldToggle(useType, m_iState == STATE_ON)) return;
...
//		vecDest.z = pev->mins.z;
	...
	CSprite *pSprite = CSprite::SpriteCreate( "sprites/effects/exp_end.spr", vecDest, true );	
