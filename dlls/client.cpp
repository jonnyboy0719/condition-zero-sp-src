...
void ClientCommand(edict_t* pEntity)
{
	...
	else if (FStrEq(pcmd, "ent_fire"))
	{
		...
		edict_t* pFind;
		pFind = FIND_ENTITY_BY_TARGETNAME(NULL, STRING(iszItem));

		while (!FNullEnt(pFind))
		{
			CBaseEntity* pEnt = CBaseEntity::Instance(pFind);
			if (pEnt)
				pEnt->Use(player, player, USE_TOGGLE, 0);

			pFind = FIND_ENTITY_BY_TARGETNAME(pFind, STRING(iszItem));
		}
	}
	else if (FStrEq(pcmd, "npcspk"))
	{
		int iszItem = ALLOC_STRING(CMD_ARGV(1)); // Make a copy of the classname
		int iszSnd = ALLOC_STRING(CMD_ARGV(2)); // Make a copy of the sound
		int iszListener = ALLOC_STRING(CMD_ARGV(3)); // Make a copy of the listener (who are we trying to speaking too?)
		...
		// Now, let's find our listener
		// If the listener is "player", then just grab the player entity.
		if (FStrEq(STRING(iszListener), "player"))
			pListener = player;
		else
		{
			pFind = FIND_ENTITY_BY_TARGETNAME(NULL, STRING(iszListener));
			while (!FNullEnt(pFind))
			{
				CBaseEntity* pEnt = CBaseEntity::Instance(pFind);
				if (pEnt)
				...
				pFind = FIND_ENTITY_BY_TARGETNAME(pFind, STRING(iszListener));
			}
		}

		if (pSpeaker)
		{
			EMIT_SOUND_DYN(ENT(pSpeaker->pev), CHAN_VOICE, STRING(iszSnd), 1, ATTN_NORM, 0, PITCH_NORM);
			// If we have a listener, then we need to look at them!
			if (pListener)
			{
				CTalkMonster* pTalkMonster = GetClassPtr((CTalkMonster*)pSpeaker->pev);
				if (pTalkMonster)
					pTalkMonster->m_hTalkTarget = pListener;
			}
		}
	}
	else if (FStrEq(pcmd, "ent_kill"))
	{
		int iszItem = ALLOC_STRING(CMD_ARGV(1)); // Make a copy of the classname
